    ...
    <div className="p-4 rounded-xl font-mono text-[10px] leading-relaxed overflow-x-auto"
      ...
      <div className="flex items-center gap-1.5 mb-3 pb-2" style={{ borderBottom: '1px solid rgba(30,41,59,0.5)' }}>
        ...
        <span className="ml-2 text-[9px]" style={{ color: '#334155', fontFamily: 'JetBrains Mono, monospace' }}>
          anveshak / static-decompiler — payload.asm
        </span>
      </div>
      {lines.map((l, i) => (
        <div
  ...
  return (
    <div className="page space-y-7 anim-fade-up">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div className="section-header" style={{ marginBottom:0, paddingBottom:0, border:'none' }}>
          <div className="section-icon" style={{ background:'var(--red-dim)', border:'1px solid var(--red-border)' }}>
            <Cpu className="w-4 h-4" style={{ color:'var(--red)' }} />
          </div>
          <div>
            <h1 className="t-title">Attachment Sandbox</h1>
            <p className="t-body mt-0.5">
              Static detonation · Process tree · Network capture · MITRE ATT&CK mapping ·&nbsp;
              <span style={{ color:'var(--red)' }}>SOC &amp; Investigator only</span>
            </p>
          </div>
        </div>
        <div className="flex items-center gap-2 flex-wrap shrink-0">
          ...
          >
            ...
            Isolated · No Execution
          </span>
          <span
            className="flex items-center gap-2 px-3 py-1.5 rounded-xl text-xs font-semibold"
            style={{ background:'var(--purple-dim)', border:'1px solid var(--purple-border)', color:'var(--purple)' }}
          >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 stagger">
        {[
          { label:'Samples',          value: attachments.length,                                   color:'var(--purple)' },
          { label:'Malware Verdict',  value: attachments.filter(a => a.risk_level === 'HIGH').length, color:'var(--red)'    },
          { label:'MITRE Techniques', value: 12,                                                   color:'var(--purple)' },
          { label:'Network IOCs',     value: 8,                                                    color:'var(--amber)'  },
        ].map(({ label, value, color }) => (
          <div key={label} className="apple-card p-4 text-center">
            <p className="t-label mb-2">{label}</p>
            <p className="font-bold" style={{ fontSize:'1.5rem', color, letterSpacing:'-0.04em' }}>{value}</p>
          </div>
        ))}
      </div>

      {/* ── Sandbox cards (inner logic untouched) ── */}
